...
While high-performance Mixed-Integer Linear Programming (MILP) solvers find mathematically exact global optimums, their triplet cycle-inequality constraint sets explode as \(O(N^3)\), making them intractable for graphs exceeding \(N=100\). Recently, Graph Neural Networks combined with Reinforcement Learning (GNN-RL) have emerged as powerful neural solvers by learning sequential edge contraction heuristics \citep{cao2025gpdqn}.

However, standard GNN-RL combinatorial partitioners are strictly monolithic: they process the entire global graph as a single batch. During backpropagation, the GNN must cache continuous activation tensors for every node across all layers, requiring over 35 GB of VRAM for large graphs. Centralized Multi-Agent RL algorithms (such as MAPPO and QMIX) also scale catastrophically due to centralized mixing networks or critics, causing immediate CUDA Out-of-Memory (OOM) failures on large graphs \citep{yu2022mappo, rashid2018qmix}.
...
To coordinate concurrent contractions along boundary nodes, we develop an event-triggered quantized GAT consensus protocol.
...
To further reduce communication latency, we deploy recurrent linear boundary estimators and dynamic event-triggered gating. The physical transmission is suppressed if the true quantized embedding remains close to the local estimator prediction:
\begin{equation}
...
We deploy localized concurrent Monte Carlo Tree Search (MCTS) planners \citep{silver2017alphazero} to guide Q-value selections, resolving non-stationarity via policy-prior neighbor transition noise.
